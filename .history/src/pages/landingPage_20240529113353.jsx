...
import leftImage from '../assets/leftimage.jpg'; // Ensure you have this image
      ...
      {isLoading ? (
        ...
        <div className="flex min-h-screen">
          <div className="w-3/5 relative">
            {show3DModel ? (
              <ThreeDImage modelUrl="/apartments.glb" /> // Ensure the path to the model is correct
            ) : (
              <div className="h-full" style={{ backgroundImage: `url(${leftImage})`, backgroundSize: 'cover' }}>
                <button
                  onClick={handleView3DModel}
                  className="bg-yellow-500 text-black px-4 py-2 rounded-md font-bold absolute bottom-4 left-4"
                >
                  View 3D Model
                </button>
              </div>
